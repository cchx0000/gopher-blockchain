...
func calculateHash(block Block) string {
	record := strconv.Itoa(block.Index) + block.Timestamp + strconv.Itoa(block.Entry) + block.PrevHash
	h := sha256.New()
	h.Write([]byte(record))
	hashed := h.Sum(nil)
...
func generateBlock(oldBlock Block, Entry int) Block {
...
func writeData(rw *bufio.ReadWriter) {
	...
	for {
		...
		entry, err := strconv.Atoi(sendData)
		if err != nil {
			log.Fatal(err)
		}
		newBlock := generateBlock(Blockchain[len(Blockchain)-1], entry)
...
func main() {
	...
	if *target == "" {
		...
		log.Println("opening stream to ",peerid)
